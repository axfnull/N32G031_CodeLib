...
4、注意事项
	此例程的IAR工程为了充分测试芯片性能需要修改堆栈，为避免和其他例程冲突，所以需要选择工程内部的icf文件，更换路径后icf文件会找不到导致编译错误，需要打开工程后选择Options->linker->config->Override default下重新选择工程内部的N32G031x6.icf文件即可，重新编译后就不会报错。
